<script lang="ts">
  ...
  function updateSuggestions() {
    ...
    if (!context.isChipName) {
      ...
      if (!usingSpecial && searchTerm.length < 2) {
        hideSuggestions()
        return
      }
  ...
  function scrollSelectedIntoView() {
    ...
    setTimeout(() => {
      const element = document.querySelector(
        `[data-placeholder-suggestion-index="${selectedSuggestionIndex}"]`
      )
...
{#if showSuggestions}
  ...
  >
    {#each suggestions as suggestion, index (index)}
      <button
        type="button"
        data-placeholder-suggestion-index={index}
