 ...
 */
function expandCustomTagsLegacy(
  tags: string[],
  model: TreeModel,
  ...
  function expandRandomObjectOfArraysTag(
    ...
    const idx = getSecureRandomIndex(arrays.length)
    const chosenArray = arrays[idx]
    const chosenPath = getNodePath(m, chosenArray.id)
    const result = expandRandomArrayTag(chosenPath, m)
    return { expandedTags: result.expandedTags, resolution: result.resolution }
  }
...
// Refactored, easier-to-follow wrapper that uses extracted helpers and a shared context
