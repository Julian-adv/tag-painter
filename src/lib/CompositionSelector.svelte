...
<div class="border-b border-gray-200">
  <h3 class="text-sm font-medium text-gray-800 p-2 text-left">Composition</h3>
  <div class="flex gap-2 flex-wrap bg-gray-200 p-2 rounded-lg">
    {#each compositions as composition (composition.id)}
      <button
        type="button"
        class="border-2 border-transparent rounded-lg p-1 cursor-pointer transition-all duration-200 flex items-center justify-center hover:border-sky-300 hover:bg-sky-50 {selectedComposition === composition.id ? 'border-sky-500 bg-sky-200' : ''}"
